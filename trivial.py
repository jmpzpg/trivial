...
def iniciar_partida(modo):
    ...
    while(nueva_partida):
        ...
        lista_obj_preg_resp = tomar_preguntas_aleatorias_de_la_bd(modo)
...
def tomar_preguntas_aleatorias_de_la_bd(modo):
    ...
    for fila in filas_preg:
        ...
        lista_obj_respuestas_de_preg = []
        for t in resp:
            tmp = Respuesta(t[0], t[1], t[2])
            lista_obj_respuestas_de_preg.append(tmp)
        lista_obj_preguntas_con_resp.append(Pregunta(fila[0], fila[1], fila[2], fila[3], lista_obj_respuestas_de_preg))
    return lista_obj_preguntas_con_resp
...
#tomar_preguntas_aleatorias_de_la_bd(Modo())
